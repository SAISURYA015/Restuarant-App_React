...
const Header = () => {
  ...
  return (
    <header className=' fixed z-50 w-screen p-3 px-4 md:p-6 md:px-16'>
      {/* desktop & tablet */}
        <div className='hidden md:flex w-full h-ful l items-center justify-between'>
            ...
            <div className='flex items-center gap-8'>
                ...
                <div className='relative'>
                    ...
                    src={user ? user.photoURL : Avatar} className='w-10 min-w-[40px] h-10 min-h-[40px] drop-shadow-xl cursor-pointer rounded-full' alt="userprofile" 
                    onClick={login}
                    />
                    {isMenu && (
                        <motion.div 
                            ...
                            className="w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-12 right-0">
                                {user && user.email === "saisurya015@gmail.com" && (
                                    <Link to={'/createItem'}>
                                        <p 
                                            className='px-4 flex items-center gap-3 cursor-pointer hover:bg-slate-200 transition-all duration-100 ease-in-out text-textColor text-base'
                                ...
                                <p className='px-4 flex items-center gap-3 cursor-pointer hover:bg-slate-200 transition-all duration-100 ease-in-out text-textColor text-base'
                                onClick={logout}
                                >Logout <MdLogout /> </p>
                        </motion.div>
                    )}
      ...
      {/* mobile */}
        <div className='flex items-center justify-between md:hidden w-full h-full'>
            ...
            <motion.div 
                ...
                {user && user.email === "saisurya015@gmail.com" && (
                    <Link to={'/createItem'}>
                        <p className='px-4 py-2 flex items-center cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-in-out text-textColor text-base' onClick={() => setisMenu(false)}>New Item <MdAdd /> 
                        </p>
                    </Link>
                )}
                <ul className='flex flex-col'>
                    ...
                    <li className='text-base text-headingColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer hover:bg-slate-100 px-4 py-2' onClick={() => setisMenu(false)}>Service</li>
                </ul>
